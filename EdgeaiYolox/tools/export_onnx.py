...
import cv2
_SUPPORTED_DATASETS = ["coco", "lm","lmo", "ycbv", "coco_kpts", "custom"]
_NUM_CLASSES = {"coco":80, "lm":15, "lmo":8, "ycbv": 21, "coco_kpts":1, "custom": 1}
_VAL_ANN = {
    ...
    "custom": "validation_annotations.json",
}
_TRAIN_ANN = {
    ...
    "custom": "images_annotations.json",
}
_SUPPORTED_TASKS = {
    "coco":["2dod"],
    "lm":["2dod", "object_pose"],
...
def main(kwargs=None, exp=None):
    ...
    if args.export_det:
        if args.task == "object_pose":
            camera_matrix_np = None
            if args.dataset == 'ycbv':
                camera_matrix_np = model.head.cad_models.camera_matrix['camera_uw']  #camera_matrix for val split
            elif args.dataset == 'custom':
                camera_matrix_np = model.head.cad_models.camera_matrix['camera_1']
            elif args.dataset == 'lmo' or args.dataset == "lm":
                camera_matrix_np = model.head.cad_models.camera_matrix
            post_process = PostprocessExport(conf_thre=0.4, nms_thre=0.01, num_classes=exp.num_classes, object_pose=True, camera_matrix=camera_matrix_np)
    ...
    if args.dataset == 'ycbv':
        ...
        img = cv2.imread("datasets/MyDataset/images/validation/1_1_000002.png") 
        assert img is not None, "Your custom sample image for ONNX export could not be found."
    else:
