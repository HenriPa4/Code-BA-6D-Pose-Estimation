...
from ..data.datasets.custom import CADModelsCustom
...
class YOLOXObjectPoseHead(nn.Module):
    def __init__(
        ...
        if "lm" in dataset:
            ...
            raise ValueError(f"Unsupported dataset type or name: {dataset}")
        self.adds = adds
    ...
    def xy2XY(self, pose):
        if isinstance(self.cad_models.camera_matrix, dict):  #has to be taken care of properly
            first_camera_key = list(self.cad_models.camera_matrix.keys())[0]
            camera_matrix = self.cad_models.camera_matrix[first_camera_key]
        else:
