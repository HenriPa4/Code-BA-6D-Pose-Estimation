...
from ..data.datasets.myDataset import MyDataset
import numpy as np
...
class ObjectPoseEvaluator:
    ...
    def __init__(
        ...
        if self.test_bop:
            ...
            elif isinstance(dataloader.dataset, MyDataset):
        ...
        for cur_iter, (imgs, targets, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if isinstance(self.dataloader.dataset.cad_models.camera_matrix, dict):
                    image_ann = self.dataloader.dataset.coco.loadImgs(int(ids[0]))[0]
                    camera_folder = image_ann.get('image_folder')
                    camera_matrix = self.dataloader.dataset.cad_models.camera_matrix[f'camera_{camera_folder}']
                else:
