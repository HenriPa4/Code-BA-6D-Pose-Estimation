...
def get_cuboid_corner(dataset="ycbv"):
    if dataset == "ycbv":
        ...
        # This approach mimics the structure of the original file.

        # 1. Define the dimensions (width, height, depth) in METERS.
        #    These MUST match the obj_dims in your myDataset.py file.
        custom_object_dims = np.array([[71.0, 69.0, 102.0]], dtype=np.float32)

        # 2. Calculate the half-dimensions, just like the ycbv_vertices array.
        #    Note: The original ycbv_vertices seems to be in millimeters, but since your
        #    obj_dims are in meters, we will stick to meters for consistency.
        custom_half_dims = custom_object_dims / 2.0

        # 3. Use NumPy broadcasting to generate the 8 corners, same as the YCBV code.
        #    The shape becomes (1, 8, 3), where the first dimension is for the object class.
        corners_3d = custom_half_dims[:, None, :] * vertices_order

        # 4. Return the dictionary keyed by your model's output class ID (which is 0).
        #    We take the first (and only) element from the generated corners.
        return {0: corners_3d[0]}
...
def get_camera_matrix(dataset="ycbv"):
    if dataset == "ycbv":
        ...
        try:
            # IMPORTANT: Make sure this file is in the same folder as your LiveFeed.py
            # or provide the full path.
            calib_data = np.load("calibration_d405.npz")
            return calib_data['camera_matrix']
        except FileNotFoundError:
            print("ERROR: calibration_d405.npz not found!")
            return None
    else:
...
def get_class_names(dataset="ycbv"):
    if dataset == "ycbv":
        ...
        # The model was trained with num_classes=2.
        # Class 0 is background (or your object if you used 0-indexing).
        # Class 1 is your object (if you used 1-indexing).
        # Let's assume you used 0-indexing as is standard.
        return {0: 'my_tracked_object'}
    else:
        return LM_CLASSES
...
def draw_obj_pose(origin_img, dets, class_names, class_to_cuboid, camera_matrix, conf_thres=0.85):
    ...
    for det in dets:
        ...
        if score>conf_thres:
            ...
            #print(f"DEBUG - Final 3D Translation (mm): {translation_vec}")
